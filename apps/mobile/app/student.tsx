...
export default function StudentScreen() {
  ...
  const apiBase = getApiUrl(PORTS.student);
    ...
    <SafeAreaView style={s.container}>
      <View style={s.topBar}>
        ...
        <View style={s.topBarCenter}>
          <View style={[s.topIcon, { backgroundColor: '#EDF7ED' }]}>
            <Ionicons name="school" size={18} color="#5A7A4E" />
          </View>
          <Text style={s.topTitle}>Student</Text>
        </View>
      ...
      <ScrollView style={s.content} contentContainerStyle={s.contentInner}>
        {connected === null && (
          <View style={s.statusCard}>
            ...
            <Text style={s.statusText}>Connecting to Student API...</Text>
          </View>
        )}
        {connected === false && (
          <View style={[s.statusCard, s.errorCard]}>
            ...
            <Text style={s.errorHint}>Start: cd apps/student/backend && npm run start</Text>
            <Text style={s.errorUrl}>{apiBase}</Text>
          </View>
        )}
        {connected === true && (
          <>
            <Text style={s.sectionTitle}>Activities & Results</Text>
            <View style={s.card}>
              <Ionicons name="book-outline" size={20} color={C.sageDark} />
              <View style={s.cardBody}>
                <Text style={s.cardTitle}>Student Dashboard</Text>
                <Text style={s.cardDesc}>Activities, results & attendance from the Student backend.</Text>
              </View>
            </View>
          </>
        )}
