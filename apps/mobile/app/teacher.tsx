...
export default function TeacherScreen() {
  ...
  const apiBase = getApiUrl(PORTS.teacher);
    ...
    <SafeAreaView style={s.container}>
      <View style={s.topBar}>
        ...
        <View style={s.topBarCenter}>
          <View style={[s.topIcon, { backgroundColor: '#E8F1F8' }]}>
            <Ionicons name="easel" size={18} color="#3F7CAC" />
          </View>
          <Text style={s.topTitle}>Teacher</Text>
        </View>
      ...
      <ScrollView style={s.content} contentContainerStyle={s.contentInner}>
        {connected === null && (
          <View style={s.statusCard}>
            ...
            <Text style={s.statusText}>Connecting to Teacher API...</Text>
          </View>
        )}
        {connected === false && (
          <View style={[s.statusCard, s.errorCard]}>
            ...
            <Text style={s.errorHint}>Start: cd apps/teacher/backend && npm run start</Text>
            <Text style={s.errorUrl}>{apiBase}</Text>
          </View>
        )}
        {connected === true && (
          <>
            <Text style={s.sectionTitle}>Classes & Syllabus</Text>
            <View style={s.card}>
              <Ionicons name="clipboard-outline" size={20} color="#3F7CAC" />
              <View style={s.cardBody}>
                <Text style={s.cardTitle}>Teacher Dashboard</Text>
                <Text style={s.cardDesc}>Classes, syllabus & tests from the Teacher backend.</Text>
              </View>
            </View>
          </>
        )}
